...
/**
 * Get hero products from heroSettings singleton
 * Returns up to 3 selected products for the hero section with rotating display
 */
...
/**
 * Filter products - ordered by newest (release date descending)
 * For New Arrivals - shows most recently added products first
 * Falls back to _createdAt if releaseDate not set
 */
export const FILTER_PRODUCTS_BY_NEWEST_QUERY = defineQuery(
  `*[${PRODUCT_FILTER_CONDITIONS}] | order(coalesce(releaseDate, _createdAt) desc) ${FILTERED_PRODUCT_PROJECTION}`
);
